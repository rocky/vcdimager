...
static void
dump_lot_and_psd_vcd(gconstpointer data, gconstpointer data2, guint32 psd_size)
{
  ...
  while((tmp = GUINT16_FROM_BE(lot->offset[n++])) != 0xFFFF) {
	...
	g_print(" [%2d]: play list descriptor ( NOI: %d | LID#: %d "
		...
		GUINT16_FROM_BE(d->ptime),
		d->wtime, d->atime);
		
	for(i = 0; i < d->noi;i++)
	  g_print(" | itemid[%d]: %d",
		  i,
		  GUINT16_FROM_BE(d->itemid[i]));
	g_print(")\n");
      }
      break;
    case PSD_TYPE_END_OF_LIST:
      {
	const PsdEndOfListDescriptor *d = (gconstpointer)(psd_data+tmp);
...
dump_info_vcd(gconstpointer data)
{
  ...
  g_print(" first segment addr: %2.2x:%2.2x:%2.2x\n", 
	  ...
	  info.first_seg_addr[1], 
	  info.first_seg_addr[2]);
...
dump_entries_vcd(gconstpointer data)
{
  ...
  for(n = 0; n < ntracks;n++) {
    ...
    g_print(" entry %d starts in track %d at (lba) extent %d (msf: %2.2x:%2.2x:%2.2x)\n",
	    n+1, from_bcd8(entries.entry[n].n), extent,
	    entries.entry[n].m, entries.entry[n].s, entries.entry[n].f);
  }
...
dump_device(const gchar device_fname[])
{
  ...
  if(psd_size) {
    ...
    psd_buf = g_malloc0(ISO_BLOCKSIZE*(((psd_size-1)/ISO_BLOCKSIZE)+1));
    
    for(n = LOT_VCD_SECTOR;n < PSD_VCD_SECTOR;n++) {
      fseek(fd, n*ISO_BLOCKSIZE, SEEK_SET);
      fread(lot_buf+(ISO_BLOCKSIZE*(n-LOT_VCD_SECTOR)), ISO_BLOCKSIZE, 1, fd);
    }

    for(n = PSD_VCD_SECTOR;n < PSD_VCD_SECTOR+((psd_size-1)/ISO_BLOCKSIZE)+1;n++) {
      fseek(fd, n*ISO_BLOCKSIZE, SEEK_SET);
      fread(psd_buf+(ISO_BLOCKSIZE*(n-PSD_VCD_SECTOR)), ISO_BLOCKSIZE, 1, fd);
    }
  }
...
dump_file(const gchar image_fname[])
{
  ...
  if(psd_size) {
    ...
    for(n = LOT_VCD_SECTOR;n < PSD_VCD_SECTOR;n++) {
      ...
      memcpy(lot_buf+(ISO_BLOCKSIZE*(n-LOT_VCD_SECTOR)), buf+8, ISO_BLOCKSIZE);
    }

    for(n = PSD_VCD_SECTOR;n < PSD_VCD_SECTOR+((psd_size-1)/ISO_BLOCKSIZE)+1;n++) {
      read_raw_mode2_sector(fd, buf, n);
      memcpy(psd_buf+(ISO_BLOCKSIZE*(n-PSD_VCD_SECTOR)), buf+8, ISO_BLOCKSIZE);
    }
  }
