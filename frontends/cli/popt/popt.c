...
void
poptSetExecPath (poptContext con, const char *path, int allowAbsolute)
{
  if (con->execPath)
    xfree (con->execPath);
...
static void
invokeCallbacks (poptContext con, const struct poptOption *table, int post)
{
  ...
  while (opt->longName || opt->shortName || opt->arg)
		...
		(post && (opt->argInfo & POPT_CBFLAG_POST))))
	{
	  ...
	  cb (con,
	      post ? POPT_CALLBACK_REASON_POST : POPT_CALLBACK_REASON_PRE,
  ...
  if (longName)
    {
      while (i >= 0 && (!con->execs[i].longName ||
			strcmp (con->execs[i].longName, longName)))
	i--;
    }
  else
    {
      while (i >= 0 && con->execs[i].shortName != shortName)
	i--;
    }
...
/* Only one of longName, shortName may be set at a time */
static int
handleAlias (poptContext con, const char *longName, char shortName,
  ...
  if (longName)
    {
      while (i >= 0 && (!con->aliases[i].longName ||
			strcmp (con->aliases[i].longName, longName)))
	i--;
    }
  else
    {
      while (i >= 0 && con->aliases[i].shortName != shortName)
	i--;
    }
...
execCommand (poptContext con)
{
  ...
  if (!con->execAbsolute && strchr (script, '/'))
    return;
...
findOption (const struct poptOption *table,
	    const char *longName,
	    char shortName,
	    poptCallbackType * callback,
	    const void **callbackData, int singleDash)
{
  const struct poptOption *opt = table;
...
{
  if (con->numLeftovers == con->nextLeftover)
    return NULL;
...
poptFreeContext (poptContext con)
{
  ...
  for (i = 0; i < con->numAliases; i++)
    {
      if (con->aliases[i].longName)
	xfree (con->aliases[i].longName);
      free (con->aliases[i].argv);
    }

  for (i = 0; i < con->numExecs; i++)
    {
      if (con->execs[i].longName)
	xfree (con->execs[i].longName);
...
int
poptAddAlias (poptContext con, struct poptAlias newAlias, int flags)
{
...
int
poptStuffArgs (poptContext con, const char **argv)
{
  int i;
